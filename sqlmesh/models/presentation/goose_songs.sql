MODEL (
    name DEV_T3_PRESENTATION.GOOSE_SONGS,
    kind FULL,
    cron '@daily',
    grain song_id
  );
...
select
    ...
    coalesce(st_earliest.show_state, st_earliest.country) as earliest_show_state,
    st_earliest.show_venue as earliest_venue,
    st_latest.show_venue as latest_venue,
    st_latest.show_city as latest_show_city,
    coalesce(st_latest.show_state, st_latest.country) as latest_show_state,
    els.plays as plays,
